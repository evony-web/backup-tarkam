/**
 * Reset Week 2 Female Tournament Data - TURSO VERSION
 * 
 * This script uses the project's db module to connect to Turso directly.
 * Run with: USE_TURSO=true bun run scripts/reset-week2-female-turso.ts
 */
...
async function resetWeek2Female() {
  console.log('=== RESET WEEK 2 FEMALE TOURNAMENT DATA (TURSO) ===\n');
  ...
  const allPointRecords = await db.playerPoint.findMany({
  ...
  for (const [playerId, totalPoints] of pointsByPlayer) {
    const player = await db.player.findUnique({ where: { id: playerId }, select: { name: true, points: true } });
  ...
  const deletedPoints = await db.playerPoint.deleteMany({
    where: { tournamentId: TOURNAMENT_ID },
  });
  ...
  for (const [playerId, changes] of playerStatChanges) {
    const player = await db.player.findUnique({ where: { id: playerId }, select: { name: true, totalWins: true, matches: true } });
  ...
  for (const [clubId, changes] of clubStatChanges) {
    const club = await db.club.findUnique({ where: { id: clubId }, select: { wins: true, losses: true, points: true, gameDiff: true } });
    ...
    await db.$executeRaw`UPDATE "Club" SET wins = MAX(wins, 0), losses = MAX(losses, 0), points = MAX(points, 0) WHERE id = ${clubId} AND (wins < 0 OR losses < 0 OR points < 0)`;
  }
  console.log('');
  ...
  const resetParts = await db.participation.updateMany({
    where: { tournamentId: TOURNAMENT_ID },
  ...
  const delPrizes = await db.tournamentPrize.deleteMany({ where: { tournamentId: TOURNAMENT_ID } });
  const delAchievements = await db.playerAchievement.deleteMany({ where: { tournamentId: TOURNAMENT_ID } });
  const delDonations = await db.donation.deleteMany({ where: { tournamentId: TOURNAMENT_ID } });
  const delSponsors = await db.tournamentSponsor.deleteMany({ where: { tournamentId: TOURNAMENT_ID } });
  const delSponsoredPrizes = await db.sponsoredPrize.deleteMany({ where: { tournamentId: TOURNAMENT_ID } });
  ...
  // ─── Step 9: Recalculate maxStreak from remaining match history ───
  console.log('Step 9: Recalculating streaks from remaining data...');
  const femalePlayers = await db.player.findMany({
    where: { division: 'female' },
    select: { id: true, name: true },
  });

  for (const player of femalePlayers) {
    const matchWins = await db.playerPoint.findMany({
      where: {
        playerId: player.id,
        reason: 'match_win',
      },
  ...
  console.log(`Matches remaining: ${await db.match.count({ where: { tournamentId: TOURNAMENT_ID } })}`);
  console.log(`Teams remaining: ${await db.team.count({ where: { tournamentId: TOURNAMENT_ID } })}`);
  console.log(`PlayerPoints remaining: ${await db.playerPoint.count({ where: { tournamentId: TOURNAMENT_ID } })}`);

  console.log('\n=== RESET COMPLETE (TURSO) ===');
}
